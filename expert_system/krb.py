rules = {
    ...
    "Performance" : [
        ...
        'Tabs(High)==> Potential(MemoryOverload)',
        'BrowsingExperience(Slow) ==> Potential(MemoryOverload)' ,
        ...
        'Potential(AntivirusInterference) & ExclusionPossible(Chrome) ==> ProblemType(AntivirusInterference)',  # F3 (new)
        'BrowsingExperience(Slow) & NewTabs(Slow) & Cpu(High) ==> Potential(ResourceContention)',  # E4 (new)
        'Potential(ResourceContention) & BackgroundProcesses(High) ==> ProblemType(BackgroundProcessesImpact)'
    ] ,
